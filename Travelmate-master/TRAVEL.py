...
def profile():
    ...
    if 'user_id' not in session:
        return redirect(url_for('login'))  # redirect guests to login

    username = session.get('username')
    user_id = session.get('user_id')

    # Optionally, fetch more user info from database
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT email, username FROM users_data WHERE user_id=%s", (user_id,))
